...
const { Comment, User, Post, ActiveLog, Notification, Mention } = require('../models'); // CommentPath 제거됨
...
router.post('/:commentId/reply', isLoggedIn, async (req, res, next) => {
  try {
    ...
    // 2단계(대댓글) 이상 차단
    if (parent.parent_id) {
      return res.status(400).send('2단계(대댓글)까지만 허용');
    }
