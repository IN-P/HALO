...
서비스 운영 중 수집될 **사용자 행동 데이터**와 **트렌드 변화**를 기반으로,  
기존 기능 개선 및 신규 기능 제안을 위한 분석 체계를 아래와 같이 설계하였으며, 현재는 **구현 예정 단계**입니다.

---

###  1. 사용자 인터랙션 분석 (예정)

- 사용자의 **좋아요, 리트윗, 댓글, 조회수** 등 활동 기록을 바탕으로  
  선호 콘텐츠와 활발한 활동 유형을 분석할 계획입니다.
- 주요 목표:
  - 인기 게시글 기반 피드 정렬 로직 개선
  - 휴면 사용자 탐지 및 마케팅 타겟팅
  - UX 최적화를 위한 사용 흐름 시각화
...
- **Google Analytics (react-ga4)**  
  : 페이지 방문자 수, 클릭률, 세션 시간 등 흐름 분석 → *비로그인 사용자 포함*
- **Matomo (자체 분석 도구)**  
  : 개인정보보호 준수를 위한 자체 호스팅형 도입 고려 중
- **Heatmap, Scrollmap 도구 연동**  
  : 사용자의 페이지 내 클릭/시선 흐름 시각화 분석 → UI 개선에 활용

> ※ 현재 위 분석 도구들은 도입만 계획된 상태이며, 데이터 수집/시각화 기능은 추후 연동 예정입니다.

---

###  3. 개선 루프 구조 (설계 완료, 구현 예정)
...
이 구조는 향후 **데이터 기반 기능 개선 체계**로서,  
단순 SNS 운영을 넘어 **지속 가능한 서비스 품질 향상 전략**으로 작용할 예정입니다.
